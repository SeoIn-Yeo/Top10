...
2025년 Top 10에는 두 개의 신규 카테고리가 추가되고 하나의 카테고리가 통합되었다. 이번 업데이트에서는 증상보다는 근본 원인에 초점을 두었다. 소프트웨어 엔지니어링과 소프트웨어 보안의 복잡성을 고려할 때, 어느 정도 중복이 없는 10개의 카테고리를 만드는 것은 사실상 불가능했다.
